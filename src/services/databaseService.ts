...
/**
 * Compares a reference image against a live capture using the Gemini service.
 */
...
export const verifyFaceWithAI = async (
    ...
    try {
        ...
        // FIX: Use the single live image directly for comparison.
        const result = await verifyFaceSimilarity(referenceImages[0], liveImage);
        
        if (result.faceSimilarityScore < SIMILARITY_THRESHOLD) {
             return { match: false, reason: `Face similarity score of ${result.faceSimilarityScore}% is below the required ${SIMILARITY_THRESHOLD}%.` };
        }

        return { match: true, reason: `Verification successful with ${result.faceSimilarityScore}% confidence.` };
